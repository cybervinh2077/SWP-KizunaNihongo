...
async function getOwnedExam(id, teacherId) {
    const { data } = await supabaseAdmin.schema('exam_module').from('quizzes')
...
exports.listMyExams = async (req, res) => {
    try {
        const { data: exams, error } = await supabaseAdmin.schema('exam_module').from('quizzes')
            .select(EXAM_FIELDS)
        ...
        const [{ data: assignments }, { data: pending }] = await Promise.all([
            supabaseAdmin.schema('classroom_module').from('exam_assignments').select('id,exam_id').in('exam_id', examIds),
            supabaseAdmin.schema('exam_module').from('quiz_attempts').select('id,quiz_id').in('quiz_id', examIds).eq('status', 'pending_review'),
        ]);
        const assignCounts = {};
...
exports.createExam = async (req, res) => {
    ...
    try {
        const { data, error } = await supabaseAdmin.schema('exam_module').from('quizzes')
            .insert({
...
exports.getExam = async (req, res) => {
    try {
        ...
        const { data: questions } = await supabaseAdmin.schema('exam_module').from('quiz_questions')
        ...
        console.error('getExam error:', err.message);
...
exports.updateExam = async (req, res) => {
    try {
        ...
        const { data, error } = await supabaseAdmin.schema('exam_module').from('quizzes').update(updates).eq('id', req.params.id).select().single();
        if (error) throw error;
        res.json(data);
    } catch (err) {
...
exports.deleteExam = async (req, res) => {
    try {
        const exam = await getOwnedExam(req.params.id, req.user.id);
        if (!exam) return res.status(403).json({ error: 'Không có quyền với đề thi này.' });
        await supabaseAdmin.schema('exam_module').from('quizzes').delete().eq('id', req.params.id);
...
exports.createQuestion = async (req, res) => {
    ...
    try {
        const { data, error } = await supabaseAdmin.schema('exam_module').from('quiz_questions')
            .insert({
                quiz_id,
...
exports.updateQuestion = async (req, res) => {
    try {
        const { data: q } = await supabaseAdmin.schema('exam_module').from('quiz_questions').select('quiz_id').eq('id', req.params.id).single();
        ...
        const updates = Object.fromEntries(Object.entries(req.body).filter(([k]) => allowed.includes(k)));
        const { data, error } = await supabaseAdmin.schema('exam_module').from('quiz_questions').update(updates).eq('id', req.params.id).select().single();
        if (error) throw error;
        res.json(data);
    } catch (err) {
...
exports.deleteQuestion = async (req, res) => {
    try {
        ...
        await supabaseAdmin.schema('exam_module').from('quiz_questions').delete().eq('id', req.params.id);
...
exports.importFromBank = async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabaseAdmin.schema('exam_module').from('quiz_questions').insert(rows).select();
        if (error) throw error;
        res.status(201).json(data);
    } catch (err) {
...
exports.listAssignments = async (req, res) => {
    try {
        ...
        const { data: assignments, error } = await supabaseAdmin.schema('classroom_module').from('exam_assignments')
        ...
        const { data: classes } = await supabaseAdmin.schema('classroom_module').from('classes').select('id,name').in('id', classIds);
...
exports.assignExam = async (req, res) => {
    ...
    try {
        ...
        const { data: cls } = await supabaseAdmin.schema('classroom_module').from('classes').select('teacher_id').eq('id', class_id).single();
        if (!cls || cls.teacher_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền với lớp học này.' });

        const { data, error } = await supabaseAdmin.schema('classroom_module').from('exam_assignments')
            .insert({
...
exports.updateAssignment = async (req, res) => {
    try {
        const { data: a } = await supabaseAdmin.schema('classroom_module').from('exam_assignments').select('exam_id').eq('id', req.params.id).single();
        ...
        const { data, error } = await supabaseAdmin.schema('classroom_module').from('exam_assignments').update(updates).eq('id', req.params.id).select().single();
        if (error) throw error;
        res.json(data);
    } catch (err) {
...
exports.deleteAssignment = async (req, res) => {
    try {
        ...
        await supabaseAdmin.schema('classroom_module').from('exam_assignments').delete().eq('id', req.params.id);
...
exports.listAttempts = async (req, res) => {
    try {
        ...
        const { data: attempts, error } = await supabaseAdmin.schema('exam_module').from('quiz_attempts')
        ...
        const [{ data: users }, { data: assignments }] = await Promise.all([
            supabaseAdmin.schema('users_module').from('users').select('id,full_name,email').in('id', userIds),
            assignmentIds.length > 0
                ? supabaseAdmin.schema('classroom_module').from('exam_assignments').select('id,class_id').in('id', assignmentIds)
        ...
        const { data: classes } = classIds.length > 0
            ? await supabaseAdmin.schema('classroom_module').from('classes').select('id,name').in('id', classIds)
            : { data: [] };
        const cMap = Object.fromEntries((classes || []).map(c => [c.id, c]));
...
exports.getAttempt = async (req, res) => {
    try {
        const { data: attempt, error } = await supabaseAdmin.schema('exam_module').from('quiz_attempts').select('*').eq('id', req.params.id).single();
        ...
        const { data: student } = await supabaseAdmin.schema('users_module').from('users').select('id,full_name,email').eq('id', attempt.user_id).single();
...
exports.gradeAttempt = async (req, res) => {
    ...
    try {
        const { data: attempt } = await supabaseAdmin.schema('exam_module').from('quiz_attempts').select('quiz_id,total_questions').eq('id', req.params.id).single();
        ...
        const { data, error } = await supabaseAdmin.schema('exam_module').from('quiz_attempts')
            .update({
...
exports.aiGradeAttempt = async (req, res) => {
    try {
        const { data: attempt } = await supabaseAdmin.schema('exam_module').from('quiz_attempts')
...
exports.listAssignedExams = async (req, res) => {
    try {
        const { data: enrollments } = await supabaseAdmin.schema('classroom_module').from('class_enrollments')
        ...
        const [{ data: exams }, { data: classes }, { data: attempts }] = await Promise.all([
            supabaseAdmin.schema('exam_module').from('quizzes').select('id,title,title_ja,description,time_limit').in('id', examIds),
            supabaseAdmin.schema('classroom_module').from('classes').select('id,name').in('id', classIds2),
...
async function getEnrolledAssignment(assignmentId, studentId) {
    const { data: assignment } = await supabaseAdmin.schema('classroom_module').from('exam_assignments').select('*').eq('id', assignmentId).single();
    if (!assignment) return { error: 'Không tìm thấy đề thi được giao.', code: 404 };

    const { data: enrollment } = await supabaseAdmin.schema('classroom_module').from('class_enrollments').select('id')
...
exports.getAssignedExam = async (req, res) => {
    try {
        ...
        const { count } = await supabaseAdmin.schema('exam_module').from('quiz_attempts').select('id', { count: 'exact', head: true })
        ...
        const { data: exam } = await supabaseAdmin.schema('exam_module').from('quizzes')
...
exports.submitExamAttempt = async (req, res) => {
    ...
    try {
        ...
        const { data: attempt, error: insErr } = await supabaseAdmin.schema('exam_module').from('quiz_attempts').insert({
...
exports.myExamResults = async (req, res) => {
    try {
        ...
        const { data, error: qErr } = await supabaseAdmin.schema('exam_module').from('quiz_attempts').select('*')
