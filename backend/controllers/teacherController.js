...
exports.createMyVocab = async (req, res) => {
  ...
  try {
    const { data, error } = await supabaseAdmin.schema('exam_module').from('teacher_vocabulary')
...
exports.updateMyVocab = async (req, res) => {
  try {
    const { data: row } = await supabaseAdmin.schema('exam_module').from('teacher_vocabulary').select('teacher_id,status').eq('id', req.params.id).single();
    ...
    const { data, error } = await supabaseAdmin.schema('exam_module').from('teacher_vocabulary').update(updates).eq('id', req.params.id).select().single();
    if (error) throw error;
    res.json(data);
...
exports.deleteMyVocab = async (req, res) => {
  try {
    const { data: row } = await supabaseAdmin.schema('exam_module').from('teacher_vocabulary').select('teacher_id').eq('id', req.params.id).single();
    if (!row || row.teacher_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền.' });
    await supabaseAdmin.schema('exam_module').from('teacher_vocabulary').delete().eq('id', req.params.id);
...
exports.createMyKanji = async (req, res) => {
  ...
  try {
    const { data, error } = await supabaseAdmin.schema('exam_module').from('teacher_kanji')
...
exports.updateMyKanji = async (req, res) => {
  const toArr = (v) => Array.isArray(v) ? v : (typeof v === 'string' ? v.split(',').map(s => s.trim()).filter(Boolean) : []);
  try {
    const { data: row } = await supabaseAdmin.schema('exam_module').from('teacher_kanji').select('teacher_id,status').eq('id', req.params.id).single();
    if (!row || row.teacher_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền.' });
    if (row.status === 'pending') return res.status(400).json({ error: 'Không thể sửa khi đang chờ duyệt.' });
    const { character, reading_on, reading_kun, meaning_vi, stroke_count, level } = req.body;
    ...
    const { data, error } = await supabaseAdmin.schema('exam_module').from('teacher_kanji').update(updates).eq('id', req.params.id).select().single();
    if (error) throw error;
    res.json(data);
  } catch (err) { res.status(500).json({ error: 'Không thể cập nhật.' }); }
};
...
exports.deleteMyKanji = async (req, res) => {
  try {
    const { data: row } = await supabaseAdmin.schema('exam_module').from('teacher_kanji').select('teacher_id').eq('id', req.params.id).single();
    if (!row || row.teacher_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền.' });
    await supabaseAdmin.schema('exam_module').from('teacher_kanji').delete().eq('id', req.params.id);
    res.json({ message: 'Đã xóa.' });
  } catch (err) { res.status(500).json({ error: 'Không thể xóa.' }); }
};
...
exports.getStats = async (req, res) => {
  ...
  try {
    ...
    const [lessonsRes, quizzesRes] = await Promise.allSettled([
      courseIds.length > 0
        ? supabaseAdmin.schema('content_module').from('lessons').select('id', { count: 'exact', head: true }).in('course_id', courseIds)
...
exports.updateCourse = async (req, res) => {
  try {
    const { data: existing } = await supabaseAdmin.schema('content_module').from('courses').select('created_by').eq('id', req.params.id).single();
