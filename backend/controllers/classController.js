...
exports.listMyClasses = async (req, res) => {
  try {
    const { data: classes, error } = await supabaseAdmin.schema('classroom_module')
        .from('classes')
        .select('*')
    ...
    const { data: enrollments } = await supabaseAdmin.schema('classroom_module')
    ...
    console.error('listMyClasses error:', err.message);
...
exports.createClass = async (req, res) => {
  ...
  try {
    const { data, error } = await supabaseAdmin.schema('classroom_module').from('classes')
...
exports.updateClass = async (req, res) => {
  try {
    const { data: existing } = await supabaseAdmin.schema('classroom_module').from('classes').select('teacher_id').eq('id', req.params.id).single();
    ...
    const { data, error } = await supabaseAdmin.schema('classroom_module').from('classes').update(updates).eq('id', req.params.id).select().single();
    if (error) throw error;
    res.json(data);
  } catch (err) {
...
exports.deleteClass = async (req, res) => {
  try {
    const { data: existing } = await supabaseAdmin.schema('classroom_module').from('classes').select('teacher_id').eq('id', req.params.id).single();
    if (!existing || existing.teacher_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền.' });
    await supabaseAdmin.schema('classroom_module').from('classes').delete().eq('id', req.params.id);
...
exports.getClassStudents = async (req, res) => {
  try {
    const { data: cls } = await supabaseAdmin.schema('classroom_module').from('classes').select('teacher_id').eq('id', req.params.id).single();
    if (!cls || cls.teacher_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền.' });

    const { data: enrollments, error } = await supabaseAdmin.schema('classroom_module')
        .from('class_enrollments')
    ...
    const { data: users } = await supabaseAdmin.schema('users_module').from('users').select('id, full_name, email, avatar_url').in('id', studentIds);
...
exports.updateEnrollmentStatus = async (req, res) => {
  ...
  try {
    const { data: enrollment } = await supabaseAdmin.schema('classroom_module').from('class_enrollments').select('class_id').eq('id', req.params.enrollmentId).single();
    if (!enrollment) return res.status(404).json({ error: 'Không tìm thấy.' });
    const { data: cls } = await supabaseAdmin.schema('classroom_module').from('classes').select('teacher_id').eq('id', enrollment.class_id).single();
    if (!cls || cls.teacher_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền.' });
    const { data, error } = await supabaseAdmin.schema('classroom_module').from('class_enrollments').update({ status }).eq('id', req.params.enrollmentId).select().single();
    if (error) throw error;
    res.json(data);
  } catch (err) {
...
exports.removeEnrollment = async (req, res) => {
  try {
    ...
    const { data: cls } = await supabaseAdmin.schema('classroom_module').from('classes').select('teacher_id').eq('id', enrollment.class_id).single();
    if (!cls || cls.teacher_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền.' });
    const { error } = await supabaseAdmin.schema('classroom_module').from('class_enrollments').delete().eq('id', req.params.enrollmentId);
    if (error) throw error;
...
exports.joinClass = async (req, res) => {
  ...
  try {
    const { data: cls, error: clsErr } = await supabaseAdmin.schema('classroom_module')
        .from('classes')
    ...
    const { data: existing } = await supabaseAdmin.schema('classroom_module').from('class_enrollments')
    ...
    if (existing) {
      ...
      await supabaseAdmin.schema('classroom_module').from('class_enrollments').update({ status: 'active' }).eq('id', existing.id);
      return res.json({ message: `Đã tham gia lại lớp "${cls.name}".` });
    }

    await supabaseAdmin.schema('classroom_module').from('class_enrollments').insert({ class_id: cls.id, student_id: req.user.id });
...
exports.listMyEnrollments = async (req, res) => {
  try {
    ...
    const { data: classes } = await supabaseAdmin.schema('classroom_module').from('classes').select('id, name, description, teacher_id, is_active').in('id', classIds);
    const teacherIds = [...new Set((classes || []).map(c => c.teacher_id))];
    const { data: teachers } = await supabaseAdmin.schema('users_module').from('users').select('id, full_name').in('id', teacherIds);
...
exports.leaveClass = async (req, res) => {
  try {
    ...
    await supabaseAdmin.schema('classroom_module').from('class_enrollments').update({ status: 'inactive' }).eq('id', existing.id);
...
exports.adminListClasses = async (req, res) => {
  ...
  try {
    let q = supabaseAdmin.schema('classroom_module').from('classes').select('*', { count: 'exact' })
    ...
    const { data: teachers } = teacherIds.length > 0
        ? await supabaseAdmin.schema('users_module').from('users').select('id,full_name,email').in('id', teacherIds)
        : { data: [] };
    const tMap = Object.fromEntries((teachers || []).map(t => [t.id, t]));

    const classIds = (classes || []).map(c => c.id);
    const { data: enrollments } = classIds.length > 0
        ? await supabaseAdmin.schema('classroom_module').from('class_enrollments').select('class_id, status').in('class_id', classIds)
        : { data: [] };
    const eCounts = {};
    (enrollments || []).forEach(e => {
...
exports.adminGetClassDetail = async (req, res) => {
  try {
    const { data: cls, error } = await supabaseAdmin.schema('classroom_module').from('classes').select('*').eq('id', req.params.id).single();
    if (error || !cls) return res.status(404).json({ error: 'Không tìm thấy lớp.' });

    const { data: teacher } = await supabaseAdmin.schema('users_module').from('users').select('id,full_name,email').eq('id', cls.teacher_id).single();
    const { data: enrollments } = await supabaseAdmin.schema('classroom_module')
    ...
    const { data: students } = studentIds.length > 0
        ? await supabaseAdmin.schema('users_module').from('users').select('id,full_name,email,avatar_url').in('id', studentIds)
        : { data: [] };
