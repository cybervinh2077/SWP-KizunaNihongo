        ...
        <div className="max-w-3xl">
          ...
          <div className="sticky top-16 z-20 -mx-2 px-2 py-3 mb-6 bg-surface/80 backdrop-blur-md border-b border-outline/30 flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="ml-auto inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold bg-sumire-purple text-white hover:opacity-90 transition-all"
            >
...
// ── Drawer chat hỏi đáp tự do — nối tạm /api/ai/chat (BE teammate có thể thay ──
//    bằng endpoint news riêng sau) ──────────────────────────────────────────────
function ChatDrawer({ open, onClose, article }) {
  ...
  // Ngữ cảnh bài đọc gửi kèm cho AI (không hiển thị trên UI)
  ...
  const send = async () => {
    ...
    setSending(true);
    try {
      // Lần gửi đầu chèn message ngữ cảnh bài đọc lên đầu lịch sử
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col h-full animate-[slidein_0.2s_ease-out]">
        {/* Header */}
